...
fit_one <- function(cause_code, outcome_label, pollutant_term, adjustment_term, raw_col, pollutant_label) {
  ...
  fml <- as.formula(paste0(
    ...
    "splines::ns(", pollutant_term, ", df = ", spline_df, ")",
    " * arf_subtype + ",
...
readr::write_csv(prediction_df, file.path(out_root, "resubmission_subtype_hazard_spectrum_spline_predictions.csv"))
...
rug_df <- bind_rows(lapply(seq_len(nrow(pollutant_specs)), function(i) {
  ...
  mutate(
    ...
    pollutant_label = factor(
      ...
      levels = levels(prediction_df$pollutant_label)
    ),
...
p <- ggplot(prediction_df, aes(x = exposure_raw, y = hazard_ratio, color = arf_subtype, fill = arf_subtype)) +
  ...
  geom_rug(
    data = rug_df,
    aes(x = exposure_raw, color = arf_subtype),
    inherit.aes = FALSE,
    sides = "b",
    alpha = 0.08,
    linewidth = 0.18,
    length = unit(0.028, "npc"),
    show.legend = FALSE
  ) +
  facet_grid(
  ...
  labs(
    title = "Spline-Based Subtype-Specific Hazards Across Air Pollution Exposure",
    subtitle = paste0(
      "Natural cubic spline curves (df = ", spline_df,
      "); HRs are relative to the median exposure with covariates held constant"
    ),
...
png_path <- file.path(fig_dir, "resubmission_subtype_hazard_spectrum_spline.png")
pdf_path <- file.path(fig_dir, "resubmission_subtype_hazard_spectrum_spline.pdf")
ggsave(png_path, p, width = 16, height = 12, dpi = 300)
ggsave(pdf_path, p, width = 16, height = 12)

message("Wrote spline subtype hazard spectrum figure:")
message(" - ", png_path)
message(" - ", pdf_path)
