...
site_path <- file.path(pooled_dir, "site_primary_effect_estimates.csv")
pooled_path <- file.path(pooled_dir, "pooled_primary_effect_estimates.csv")
if (!file.exists(site_path)) stop("Missing site primary estimates: ", site_path)
if (!file.exists(pooled_path)) stop("Missing pooled primary estimates: ", pooled_path)
...
plot_df <- bind_rows(site_effects, pooled_effects) %>%
  filter(analysis_family %in% c("Primary mortality", "Primary VFD")) %>%
  filter(model %in% c("NO2 single-pollutant", "PM25 single-pollutant", "PM25 + NO2")) %>%
  filter(!str_detect(outcome, regex("in-hospital|imv|invasive|ventilation duration", ignore_case = TRUE))) %>%
  mutate(
    ...
    outcome_panel = case_when(
      analysis_family == "Primary mortality" ~ "Mortality by day 28",
      analysis_family == "Primary VFD" ~ "Ventilator-free days"
    ),
    x_title = case_when(
      analysis_family == "Primary mortality" ~ "Adjusted odds ratio (95% CI)",
      analysis_family == "Primary VFD" ~ "Adjusted ratio of means (95% CI)"
    ),
    pollutant_label = recode(pollutant, "NO2" = "NO2", "PM2.5" = "PM2.5"),
    estimand = case_when(
    ...
    point_size = if_else(row_type == "Pooled", 3.1, 2.25),
    line_width = if_else(row_type == "Pooled", 0.85, 0.46)
  ) %>%
  filter(!is.na(estimand), is.finite(estimate), is.finite(conf_low), is.finite(conf_high))

readr::write_csv(plot_df, file.path(pooled_dir, "pooled_primary_effect_forest_publication_ready_data.csv"))
...
theme_primary_forest <- function(base_size = 11) {
...
make_panel <- function(df, panel_title, x_limits, x_breaks, show_y = TRUE) {
...
mortality_df <- plot_df %>% filter(outcome_panel == "Mortality by day 28")
vfd_df <- plot_df %>% filter(outcome_panel == "Ventilator-free days")

p_mortality <- make_panel(
  mortality_df,
  "Mortality by day 28",
  x_limits = c(0.45, 4.25),
  x_breaks = c(0.5, 0.75, 1, 1.5, 2, 3, 4),
  show_y = TRUE
)
...
p_vfd <- make_panel(
  ...
  x_limits = c(0.74, 1.20),
  x_breaks = c(0.75, 0.85, 0.95, 1, 1.05, 1.15),
  show_y = FALSE
)

p <- (p_mortality | p_vfd) +
  plot_layout(widths = c(1.12, 1), guides = "collect") &
  theme(legend.position = "bottom")

save_plot_set <- function(plot, stem, target_dir) {
  ggsave(file.path(target_dir, paste0(stem, ".pdf")), plot, width = 10.8, height = 7.4, units = "in", device = cairo_pdf)
  ggsave(file.path(target_dir, paste0(stem, ".png")), plot, width = 10.8, height = 7.4, units = "in", dpi = 450)
}

save_plot_set(p, "pooled_primary_effect_forest_publication_ready", fig_dir)
save_plot_set(p, "pooled_primary_effect_forest", fig_dir)
save_plot_set(p, "Supp_Figure_primary_effect_forest", journal_dir)

message("Saved publication-ready primary effect forest to: ", file.path(fig_dir, "pooled_primary_effect_forest_publication_ready.png"))
